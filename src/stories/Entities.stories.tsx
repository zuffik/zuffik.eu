...
const skillsCount = database.languages.length + database.frameworks.length + database.utilities.length + database.platforms.length;
const skills = [...database.languages, ...database.frameworks, ...database.platforms, ...database.utilities];
export const skill = () => {
    const index = number('Skill', 0, {min: 0, max: skillsCount - 1, range: true});
    return (
